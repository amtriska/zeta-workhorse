...
**Tropical trace**
* **`tropical_trace(A, max_k, mode)`**: Requires `A` (NumPy array), `max_k` (integer), and `mode` (min or max). Evaluates the tropical trace sequence $Z_{trop}(1), \ldots, Z_{trop}(\text{max\_k})$ using min-plus matrix powers. Structural zeros are automatically converted to infinity. Returns a list of floats. Runtime scales as $O(\text{max\_k} \cdot V^3)$; keep `max_k` small for large graphs.